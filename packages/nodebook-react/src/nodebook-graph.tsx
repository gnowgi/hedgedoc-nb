 ...
 */
import { AsyncLoadingBoundary } from './compat/async-loading-boundary'
import HighlightedCode from './compat/highlighted-code'
import { ApplicationErrorAlert } from './compat/application-error-alert'
import type { CodeProps } from './compat/types'
import { cypressId } from './compat/cypress-attribute'
import { Logger } from './compat/logger'
import {
  getOperationsFromCnl,
  expressionToPetriNetOps,
  MorphRegistry,
  evaluateExpression,
  operationsToGraph,
...
/** Detect dark mode from the document (Bootstrap or standalone). */
function isDarkMode(): boolean {
  if (typeof document === 'undefined') return false
  // Check explicit theme attributes first
  const bsTheme = document.body.getAttribute('data-bs-theme')
  const appTheme = document.documentElement.getAttribute('data-theme')
  if (bsTheme) return bsTheme === 'dark'
  if (appTheme) return appTheme === 'dark'
  // Only fall back to OS preference if no explicit theme is set
 ...
 */
export interface NodeBookGraphProps extends CodeProps {
  printMode?: boolean
}
...
export const NodeBookGraph: React.FC<NodeBookGraphProps> = ({ code, printMode = false }) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void (async () => {
      const allExtraOps: CnlOperation[] = []
      ...
      <div className={styles['nodebook-container']} {...cypressId('nodebook-frame')}>
        ...
        <div className={`${styles['export-buttons']} d-print-none`}>
          ...
          <button
            ...
            title={
              graphInteractionEnabled ? 'Lock graph (disable pan & zoom)' : 'Unlock graph (enable pan & zoom)'
            }>
            <IconMouse size={14} />
          </button>
